...
bool MainWnd::Create() {
  ...
  wnd_ =
      ::CreateWindowExW(WS_EX_OVERLAPPEDWINDOW, kClassName, L"WebrtcPusher",
...
void MainWnd::StartLocalRenderer(webrtc::VideoTrackInterface* local_video) {
  local_renderer_.reset(new VideoRenderer(handle(), 0, 1, local_video));
}
...
MainWnd::VideoRenderer::VideoRenderer(
  ...
  // 设置接收视频数据的对象，如果想要接收视频数据，需要实现VideoSinkInterface，
  // 此接口暴露了OnFrame函数，只要将Sink通过AddOrUpdateSink函数注册给Source，
  // 那么Source就会通过OnFrame接口将数据传给Sink。
